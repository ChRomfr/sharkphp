...
class Basenewscontroller extends Controller{
	...
	public function indexAction(){
		...
		$this->app->smarty->assign(array( 
			'news'			=>	$news,
			'pagination'	=>	$NewsInDb > $this->app->config['news_per_page'] ? getPagination( array('perPage'=>$this->app->config['news_per_page'], 'fileName'=>getLink('news') .'?page=%d', 'totalItems'=>$NewsInDb) ) : null
		));
	...
	public function lockCommentaireAction($id){
		if( ADM_NEWS_LEVEL < $_SESSION['utilisateur']['isAdmin']){
			...
			header('location:'. getLink("news/view/". $id) );
			exit;
		}
