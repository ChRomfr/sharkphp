	...
	*/
		...
		if( $this->app->HTTPRequest->postExists('user') ){
			...
			if( $this->app->config['user_activation'] != 'auto'):
				...
				if( $this->app->config['user_activation'] == 'mail'):
					...
					return $this->redirect(getLink('index'), 3, "Votre compte a ete cree, vous devez maintenant l activer en cliquant sur le lien dans l'email qui vient d'etre envoyer");
				endif;
				
				# Cas admin
				if( $this->app->config['user_activation'] == 'admin'):
					return $this->redirect(getLink('index'), 3, "Votre compte a ete cree, il doit maintenant etre active par un administrateur");
				endif;
	...
	public function editAction(){
		...
		if(  $this->app->HTTPRequest->postExists('user') ):
			...
			return $this->redirect( getLink('utilisateur'),3, 'Profil mise a jour' );
		endif;
	...
	public function activationAction( ){
		...
		if(  $this->app->HTTPRequest->getExists('reg') == false ):
			return $this->redirect( getLink('index'),0,'' );
		endif;
	...
	public function changePasswordAction(){
		...
		if( $this->app->HTTPRequest->postExists('password') ):
			...
			return $this->redirect(getLink('utilisateur'), 3, $this->lang['Mot_de_passe_change']); 
		endif;
